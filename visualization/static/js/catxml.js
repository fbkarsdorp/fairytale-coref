...
// class Location
function Location(loc_xml) {
    this.name = loc_xml.getAttribute("name");
    this.id = parseInt(loc_xml.getAttribute("id"));
    this.group = parseInt(loc_xml.getAttribute("group"));
}
...
function read_locs(xml) {
    var loc_elms = xml.getElementsByTagName("location");
    locations = [];
    for (var i = 0; i < loc_elms.length; i++) {
        locations[i] = new Location(loc_elms[i]);
    }
    return locations
}
